//
//  PaginationNumberViewController.m
//  MyPebbleFaces
//
...
@interface PaginationNumberViewController ()

@end

@implementation PaginationNumberViewController
...
- (void)viewDidLoad
{
    ...
    self.title = @"Paging Number of Apps";
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (![defaults objectForKey:@"pageNumber"] ) {
        ...
        int num = [[defaults objectForKey:@"pageNumber"] intValue];
        NSLog(@"%d",num);
        int row = num/4;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:[NSString stringWithFormat:@"%d",textLabel] forKey:@"pageNumber"];
